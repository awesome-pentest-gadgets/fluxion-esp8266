...
SSIDList::SSIDList()
{
	if ( listAdr + SSIDListLength * SSIDLength > 4096 )
	{ Serial.println ( "WARNING: EEPROM overflow!" ); }
}
...
void SSIDList::addClone ( String name, int num, bool enc )
{
	...
	for ( int c = 0; c < SSIDListLength && c < num; c++ ) {
		...
		if ( c < _restSSIDLen ) {
			for ( int d = 0; d < _restSSIDLen - c; d++ ) {
				_apName += " ";  //e.g. "SAMPLEAP   "
			}
		}
			...
			if ( c < _restSSIDLen * 2 ) {
				...
				for ( int d = 0; d < ( _restSSIDLen - 3 ) - c / 2; d++ )
				{ _apName += " "; }       //e.g. "SAMPLEAP   " (zero widtdh char at the beginning)
			}
				...
				if ( c < _restSSIDLen * 3 ) {
					...
					for ( int d = 0; d < ( _restSSIDLen - 1 ) - c / 3; d++ )
					{ _apName += " "; }       //e.g. ".SAMPLEAP   "
				}
				
				else {
					for ( int d = 0; d < _restSSIDLen - 2; d++ ) {
						_apName += " ";
					}
...
String SSIDList::get ( int num )
{
	...
	for ( int i = 0; i < SSIDLength; i++ ) {
		if ( names[num][i] != 0x00 && names[num][i] != 0x10 )
		{ _name += names[num][i]; }
	}
...
void SSIDList::_random()
{
	...
	for ( int i = len; i < SSIDListLength; i++ ) {
		...
		for ( int h = 0; h < SSIDLength; h++ )
		{ _rName += letters[random ( 0, sizeof ( letters ) )]; }
