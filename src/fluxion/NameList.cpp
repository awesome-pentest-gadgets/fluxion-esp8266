...
NameList::NameList()
{
	if ( ( listLength * nameLength + 6 ) + 1 > maxSize )
...
void NameList::save()
{
	...
	for ( int i = 1; i < len * ( nameLength + 6 ) + 1; i += nameLength + 6 ) {
		for ( int h = 0; h < 6; h++ )
		{ EEPROM.write ( i + h, clients._get ( num )._get ( h ) ); }
		
		for ( int h = 0; h < nameLength; h++ )
		{ EEPROM.write ( i + h + 6, names[num][h] ); }
...
{
	if ( clients.add ( client ) >= 0 )
	{ len++; }
	
	else
		if ( clients.getNum ( client ) < 0 )
		{ Serial.println ( "WARNING: name list is full!" ); }
	...
	for ( int i = 0; i < nameLength; i++ ) {
		if ( i < name.length () )
		{ names[clients.getNum ( client )][i] = _buf[i]; }
		
		else
		{ names[clients.getNum ( client )][i] = 0x00; }
	}
