...
bool ClientScan::stop()
{
	...
	if ( curTime - startTime >= ( timeout * 1000 ) / channelsNum &&
		curChannel < channelsNum ) {
		if ( debug )
		...
		if ( curTime - startTime >= timeout * 1000 ) {
			sniffing = false;
			wifi_promiscuous_enable ( 0 );
			Serial.println ( "stopping client scan after " + ( String ) (
...
int ClientScan::add ( Mac adr )
{
	...
	if ( clientNum == -1 && results < maxClientScanResults ) {
		data_getVendor ( adr._get ( 0 ), adr._get ( 1 ),
			adr._get ( 2 ) ).toCharArray ( vendors[results], 9 );
...
size_t ClientScan::getSize()
{
	...
	for ( int i = 0; i < results && i < maxClientScanResults; i++ ) {
		...
		json += "\"n\":\"" +
			( String ) nameList.getByMac ( getClientMac ( i ) ) +
			"\",";
		json += "\"l\":" +
			( String ) nameList.getNumByMac ( getClientMac ( i ) ) +
			",";
		...
		if ( ( i != results - 1 ) && ( i != maxClientScanResults - 1 ) ) {
			json += ",";
		}
...
void ClientScan::send()
{
	...
	if ( debug )
	{ Serial.println ( "getting settings json (" + ( String ) _size + ")" ); }
